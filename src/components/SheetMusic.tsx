...
import { Renderer, Stave, StaveNote, Voice, Formatter, Accidental, Beam, StaveConnector, BarNote } from 'vexflow';
import { MidiData } from '../types/midi';
import { midiNoteToVexFlowNote } from '../utils/noteMapping';
...
const SheetMusic: React.FC<SheetMusicProps> = ({ midiData, onNoteClick }) => {
  ...
  const LINES_PER_PAGE = 6; // Reduced for better spacing
  const MEASURES_PER_PAGE = MEASURES_PER_LINE * LINES_PER_PAGE;
  const MEASURE_WIDTH = 320; // Increased for better spacing
  const SYSTEM_HEIGHT = 200; // Increased for better spacing
  ...
  // Enhanced duration calculation with better quantization
  ...
  const quantizeTime = (time: number, ticksPerQuarter: number): number => {
    const thirtySecondNote = ticksPerQuarter / 8; // Finer quantization
    return Math.round(time / thirtySecondNote) * thirtySecondNote;
  };

  // Much better note processing with intelligent filtering
  const processNotesIntoMeasures = (notes: ProcessedNote[], ticksPerQuarter: number) => {
    const measureLength = ticksPerQuarter * 4;
    ...
    const validNotes = sortedNotes.filter((note, index) => {
      ...
      const maxDuration = measureLength * 2; // Maximum 2 whole notes
    ...
    Array.from(notesByMeasure.entries())
      ...
      .forEach(([measureIndex, measureNotes]) => {
        ...
        const maxNotesPerMeasure = 16; // Reduced for cleaner look
  ...
  // Enhanced voice creation with better spacing
  const createVoiceWithNotes = (notes: StaveNote[], clef: 'treble' | 'bass') => {
    ...
    try {
      ...
      for (let line = 0; line < linesOnPage; line++) {
        ...
        for (let measureIdx = lineStartMeasure; measureIdx < lineEndMeasure; measureIdx++) {
          ...
          // Render treble notes
          const trebleVoice = createVoiceWithNotes(trebleNotes, 'treble');
          if (trebleVoice) {
            try {
          ...
          // Render bass notes
          const bassVoice = createVoiceWithNotes(bassNotes, 'bass');
          if (bassVoice) {
            try {
              const formatter = new Formatter();
    ...
    try {
      const processedNotes: ProcessedNote[] = midiData.notes
        ...
        .map(note => ({
          ...
          vexNote: midiNoteToVexFlowNote(note.noteNumber),
      ...
      const measures = processNotesIntoMeasures(processedNotes, midiData.ticksPerQuarter);
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-lg">
      ...
      <div className="mt-4 text-sm text-gray-600">
        ...
        <p className="mt-1">Clean professional sheet music with intelligent note filtering</p>
